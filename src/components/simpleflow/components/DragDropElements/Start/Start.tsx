...
export const StartDraggable = () => {
  return (
    <Draggable id="start" payload={config.payload}>
      Start Node
    </Draggable>
  )
}
...
export const StartNode = ({ id, payload }: StartNodeProps) => {
  return (
    <Node id={id} key={id} position={payload.position}>
      <div style={{ height: '4rem', width: '10rem' }}>
        <FlexContainer width="inherit" height="inherit" flexDirection="row" borderWidth="m" borderRadius="m">
          <FlexContainer width="33%" height="100%" justifyContent="center" alignItems="center">
            Icon
          </FlexContainer>
          <FlexContainer width="auto" height="25%">
            Start Node
          </FlexContainer>
        </FlexContainer>
      </div>
...
export const config: BaseDragAndDropConfig<StartData> = {
  ...
  payload: { data: { text: 'Start' }, position: defaultPosition, type },
  type
}
