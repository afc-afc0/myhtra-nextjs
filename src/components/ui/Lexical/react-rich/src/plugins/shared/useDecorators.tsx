...
export function useDecorators(editor: LexicalEditor, ErrorBoundary: ErrorBoundaryType): Array<React.JSX.Element> {
  const [decorators, setDecorators] = useState<Record<NodeKey, React.JSX.Element>>(() => editor.getDecorators<React.JSX.Element>())

  // Subscribe to changes
  useLayoutEffect(() => {
    return editor.registerDecoratorListener<React.JSX.Element>((nextDecorators) => {
      flushSync(() => {
