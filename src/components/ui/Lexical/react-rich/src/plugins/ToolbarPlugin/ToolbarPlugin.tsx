...
import { $findMatchingParent, $getNearestBlockElementAncestorOrThrow, $getNearestNodeOfType, $isEditorIsNestedEditor, mergeRegister } from '@lexical/utils';
    ...
    <ToolbarSelectContainer>
      <Select value={codeLanguage} disabled={disabled} onValueChange={(value) => onCodeLanguageSelect(value)} defaultValue={codeLanguage}>
        <SelectTrigger size='xs' aria-label='Select Language'>
          <SelectValue aria-label={codeLanguage}>
            {CODE_LANGUAGE_OPTIONS[codeLanguage]?.label || '-'}
          </SelectValue>
        </SelectTrigger>
        <SelectContent>
...
const ToolbarSelectContainer = ({ children } : { children: React.ReactNode }) => {
  return (
    <div className={styles.toolbarSelectContainer}>
      { children }
    </div>
  )
}
...
const BlockFormatDropDown =({
  ...
  return (
    <ToolbarSelectContainer>
      <Select value={blockType.toString()} defaultValue={blockTypes.paragraph.value} onValueChange={(value) => handleBlockTypeChange({ value })} disabled={disabled}>
