...
export const AuthController = () => {
  ...
  if (status === 'loading') {
    ...
    return <Button text="Sign In/Up" onClick={() => signIn('keycloak')} size="m" />
  }
}
  ...
  return (
    <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
      <PopoverTrigger asChild>
        <Toggle isPressed={isPopoverOpen} icon={<AvatarSVG />} size="m" aria-label="Profile" />
      </PopoverTrigger>
...
const ProfilePopoverContainer = ({ children }: { children: React.ReactNode }) => {
  return <div className={styles.popoverContentContainer}>{children}</div>
}
